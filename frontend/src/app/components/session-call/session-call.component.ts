...
import { WebrtcUtils } from 'src/app/_services/webrtc-utils.service';
...
export class SessionCallComponent implements OnInit, OnDestroy, MessageSender {
  ...
  isStarted!: boolean;
  shareScreenFlag: boolean = false;
  remoteSharing: boolean = false;
  ...
  shareScreen() {
    ...
    if (!this.shareScreenFlag) {
      navigator.mediaDevices.getDisplayMedia({
        ...
        let that = this;
        this.addLocalStream(stream);
        this.localStream.getVideoTracks()[0].onended = function () {
          // doWhatYouNeedToDo();
          console.log("stopSharing");
          that.shareScreenFlag = false;
          navigator.mediaDevices.getUserMedia({
            audio: true,
            video: true
          })
            .then((stream: MediaStream) => {
              that.addLocalStream(stream);
              that.peerConnection.addTrack(that.localStream.getVideoTracks()[0], that.localStream);
              that.peerConnection.addTrack(that.localStream.getAudioTracks()[0], that.localStream);
    ...
    this.shareScreenFlag = !this.shareScreenFlag;/*  */
  ...
  initiateCall(): void {
    ...
    if ((!this.isStarted && this.localStream && this.isChannelReady) || this.shareScreenFlag || this.remoteSharing) {

      if (!this.remoteSharing) this.createPeerConnection();

      this.peerConnection.addTrack(this.localStream.getVideoTracks()[0], this.localStream);
      this.peerConnection.addTrack(this.localStream.getAudioTracks()[0], this.localStream);

      //this.peerConnection.addTrack(this.localStream.getVideoTracks()[0]);
      //this.peerConnection.addTrack(this.localStream.getAudioTracks()[0]);
  ...
  createPeerConnection(): void {
    ...
    try {
      if (useWebrtcUtils && !this.shareScreenFlag) {
        this.peerConnection =
          WebrtcUtils.createPeerConnection(environment.iceServers, 'unified-plan', 'balanced', 'all', 'require', '', [], 0);
      } else {
      ...
      this.peerConnection.ontrack = (event: RTCTrackEvent) => {
        //controllare num di streams

        console.log("ON TRACK: ", event.streams.length);
        console.log("ON TRACK1: ", event.streams);
        if (event.streams[0]) {
  ...
  handleRemoteHangup(): void {
    ...
    alert('Remote client has left the call.', '');
